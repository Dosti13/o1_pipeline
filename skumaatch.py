...
=============================================================================================================
VERSION 5.1 — CACHED / PARQUET-BACKED, WITH BARCODE-POISONING FIXES  (adapted from V5)
─────────────────────────────────────────────────────────────────────────────────────
SAME MATCHING LOGIC AS V5, plus a fix for a real bug that was observed in production:

  BUG (V5): a Fuzzy/Semantic match with confidence ≥ BACKFILL_MIN_CONFIDENCE could
  "teach" a customer barcode onto the wrong LULU source row even when the two rows'
  WEIGHT or BRAND clearly disagreed (e.g. 70g vs 130g, "ULKER" vs no-brand). Once
  learned, that wrong barcode→source mapping was permanent for the rest of the run
  and got silently re-used by every later Barcode-method match. On top of that, the
  per-customer JSON cache baked in the "Our Barcode" display value AT THE TIME each
  customer was processed — so if customer A ran (and got cached) BEFORE customer B
  taught a new barcode onto a source row, A's cached output kept showing a blank/old
  "Our Barcode" forever, even on later re-runs, because a cache hit skips recompute.

FIXES IN THIS VERSION (search for "★ FIX" to find each one):
  ★ FIX 1 — Sanity-gated backfill: a Fuzzy/Semantic match may only teach a barcode
    onto a source row if weight does NOT conflict (weights_match() is not False)
    AND brand does not conflict (if both sides have brand tokens, they must overlap).
    This is the root-cause fix — it stops bad matches from poisoning the barcode
    index in the first place.
  ★ FIX 2 — Original vs Learned tracking: every source row now has a boolean
    src_barcode_is_learned[i] flag. Rows that had a real LULU barcode from the
    source file are 'Original'; rows whose barcode only exists because some
    customer's fuzzy/semantic match backfilled it are 'Learned (Backfilled)'.
    This is written out as a new 'Barcode Source' column on every matched row.
  ★ FIX 3 — Stale-cache-proof finalize pass: matched rows no longer trust the
    'Our Barcode' / 'Barcode Source' values baked in at match time (whether fresh
    or loaded from cache). Instead every matched row carries a stable '_src_i'
    (source row index), and a finalize_barcode_fields() pass re-resolves
    'Our Barcode' and 'Barcode Source' from the CURRENT src_barcode_display /
    src_barcode_is_learned arrays right before writing the Excel file — after
    every customer (including cached ones) has had a chance to teach barcodes.
    This guarantees the output always reflects the final, fully-taught state,
    regardless of which order customers were processed/cached in.
  ★ FIX 4 — Learned Barcodes Audit sheet: every successful backfill is logged
    (which barcode, which source row, which customer/row taught it, at what
    confidence) to an audit trail, cached per-customer alongside the match
    results, and combined into a 'Learned Barcodes Audit' sheet in the final
    Excel output — so a bad learn is now visible and reviewable instead of silent.
  ★ FIX 5 — Cache key now also depends on a hash of the LULU source catalogue
    itself (names + barcodes), not just the customer's own data + config knobs.
    Previously, if you edited/replaced the source Excel file, every customer's
    stale per-customer match-result cache would still hit (since customer data
    hadn't changed) and silently serve results computed against the OLD source
    catalogue and OLD source row indices. Now a source-side change invalidates
    every customer cache too.

Everything else (candidate selection, fuzzy scorer, semantic layer, barcode
pre-pass, brand pre-gate, filter logic, parquet cache, embedding cache, output
schema) is UNCHANGED from V5.
...
# alone is no longer sufficient — see FIX 1 (weight/brand sanity gate) below.
...
def load_customer_cache(path, barcode_to_src, src_barcode_display, src_barcode_is_learned):
    ...
    taught the catalogue last time, so downstream customers still benefit, and
    keeps the Original/Learned flag in sync (★ FIX 2)."""
...
# ===========================================================================
# ★ FIX 3 — finalize barcode fields AFTER all customers have run, so every
# matched row shows the FINAL, fully-taught barcode state regardless of
# customer processing order or cache hits.
# ===========================================================================
...
def main():
    ...
    # ★ FIX 2 — every source row starts as "Original" if it already had a real
    # barcode in the LULU source file; only rows that get a barcode via
    # _maybe_backfill() later flip this to True ("Learned").
    ...
    # caches (previously only customer-side data changes did).
    ...
    config_sig = _hash_list([
        ...
        SEMANTIC_MODEL, ENSEMBLE_FUZZY, USE_SEMANTIC, src_sig,
    ])
    ...
    # ★ FIX 3 — now that EVERY customer (fresh or cached) has had a chance to
    # teach barcodes, re-resolve 'Our Barcode' / 'Barcode Source' on every
    # matched row from the final, fully-taught state. This is what actually
    # eliminates the stale-blank-barcode symptom, independent of processing
    # order or cache hit/miss.
