...
CONFIG = {
    ...
    "thresholds": {
        "fuzzy": 85,     # name fuzzy score threshold
        "tfidf": 75,     # TF-IDF cosine % threshold
        "semantic": 70,  # semantic cosine % threshold
    },
...
def parse_description(text: str) -> tuple[str, str, str]:
    ...
    # last weight token (usually the unit weight)
    weight = ""
    ...
    # pack pattern like 4x6, 12x, 2x3x
    pack = ""
...
def weight_compatible(our_w: Optional[float], cust_w: Optional[float]) -> bool:
    """Both missing → ok. Both present → must be equal. Otherwise → not ok."""
...
def weight_match_label(our_w, cust_w) -> str:
    """Human-readable tag for the final report."""
    if our_w is None and cust_w is None:
...
def fuzzy_top_k(target_name: str, source_names: list[str], k: int, threshold: int):
    """Return list of (source_index, score 0–100) sorted by score desc."""
    if not target_name:
        return []
    ...
    # rapidfuzz returns (match_str, score, index)
...
class TFIDFMatcher:
    def __init__(self, source_names: list[str]):
        ...
        # char_wb captures sub-word similarity (good for typos / brand variations)
...
class SemanticMatcher:
    def __init__(self, source_names: list[str], model_name: str, batch_size: int):
    ...
    def encode(self, names: list[str], batch_size: int) -> np.ndarray:
...
def select_best_candidate(
    ...
    cust_p: Optional[str],
    source_parsed: list[dict],
    weight_rule: str = "strict",
):
    """
    Apply the priority rule from your fuzzy_match v2:
      • Among candidates, keep those whose WEIGHT matches the customer's
      • Among those, prefer the ones whose PACK also matches
      • Pick highest text-score among survivors
    Returns (best_idx, best_score, weight_matched: bool) or (None, 0, False).
    """
    if not candidates:
    ...
    if weight_ok:
        # Prefer pack match within weight-matched group
    ...
    # No weight-matched candidate
    if weight_rule == "strict":
        return None, 0.0, False

    # Soft mode: fall back to best name match without weight
    best_idx, best_score = max(weight_bad, key=lambda x: x[1])
    return best_idx, best_score, False


# ─────────────────────────────────────────────────────────────────────────────
# FILE I/O  (kept the smart-header-finder from your matchiq.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def run_pipeline(cfg: dict):
    ...
    for tgt_cfg in cfg["targets"]:
        ...
        # Pre-compute semantic embeddings for all targets at once (efficient)
        target_emb = None
        ...
        rows = []
        for i, tp in enumerate(tqdm(target_parsed, desc=f"Matching {file_name[:30]}",
                                    unit="row")):
            cust_raw = tp["raw"]
            ...
            if best["idx"] is None and semantic is not None and cust_name:
                cands = semantic.top_k_for_embedding(target_emb[i], topk["semantic"], sem_th)
                idx, sc, w_ok = select_best_candidate(
                    cands, cust_w, cust_p, source_parsed, weight_rule)
                if idx is not None:
            ...
            if best["idx"] is not None:
                ...
                rows.append({
                    ...
                    "Score": 0.0,
                    "Method": "None",
                    "Weight Match": "N/A (no weight on either side)" if cust_w is None
                                    else "Without Weight",
                })
    ...
    log.info("  Matched     : %d (%.1f%%)", matched_total,
             matched_total / total * 100 if total else 0)
