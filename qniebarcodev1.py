...
=============================================================================================================
VERSION 6.1 — STANDALONE SOURCE-OF-TRUTH CATALOGUE, WITH BARCODE-POISONING FIXES  (adapted from V6)
─────────────────────────────────────────────────────────────────────────────────────────────────────
SAME MATCHING LOGIC AS V6 (standalone catalogue file, LULU now just another
customer, mgrp_descr-first brand resolution, prdh_descr_1/2 hierarchy bonus,
all three caching layers) — PLUS the barcode-poisoning fixes that were
missing from V6 (it was built on the same base as the unfixed V5).

THE BUG (present in the V6 you sent): a Fuzzy/Semantic match with confidence
≥ BACKFILL_MIN_CONFIDENCE could "teach" a customer barcode onto the wrong
catalogue row even when weight or brand clearly disagreed. Once learned,
that wrong mapping was permanent for the run and reused by every later
Barcode-method match. On top of that, the per-customer JSON cache baked in
"Our Barcode" at the time each customer was processed — so a customer cached
BEFORE another customer taught a new barcode kept showing a stale/blank
"Our Barcode" forever, even after re-runs, because a cache hit skips
recompute entirely.

FIXES IN THIS VERSION (search "★ FIX" to find each one):
  ★ FIX 1 — Sanity-gated backfill: a Fuzzy/Semantic match may only teach a
    barcode onto a catalogue row if weight does NOT conflict
    (weights_match() is not False) AND brand does not conflict (if both
    sides have brand tokens, they must overlap). Root-cause fix.
  ★ FIX 2 — Original vs Learned tracking: every catalogue row has a
    src_barcode_is_learned[i] flag. Written out as a new 'Barcode Source'
    column (Original / Learned (Backfilled) / N/A) on every matched row.
  ★ FIX 3 — Stale-cache-proof finalize pass: matched rows carry a stable
    '_src_i' (catalogue row index) instead of a frozen barcode value. A
    finalize_barcode_fields() pass re-resolves 'Our Barcode' / 'Barcode
    Source' from the CURRENT arrays after every customer (cached or fresh)
    has had a chance to teach barcodes — output always reflects the final
    state regardless of processing order.
  ★ FIX 4 — Learned Barcodes Audit sheet: every successful backfill is
    logged (barcode, catalogue row, which customer/row taught it, method,
    confidence), cached per-customer, and combined into a new 'Learned
    Barcodes Audit' sheet in the output workbook.
  ★ FIX 5 — (carried over from V6, unchanged) config_sig already folds in
    src_file_key (the catalogue file's size+mtime hash), so replacing the
    catalogue invalidates old per-customer caches too.
  ★ FIX 6 — "Borrowed Barcode" now also flags rows matched by Method =
    'Barcode' / 'Barcode (Bridged)' whenever the catalogue row's barcode is
    itself a Learned (Backfilled) one — not just rows that just got
    backfilled via a Fuzzy/Semantic hit in THIS pass. Previously, once a
    catalogue row's barcode had been learned, every later customer that
    matched it via the (now-fast) direct barcode lookup showed Method =
    'Barcode' with 100% confidence and a BLANK 'Borrowed Barcode' column —
    silently hiding the fact that the barcode being matched against was
    never actually in the source-of-truth catalogue to begin with. This is
    resolved in two places: (a) the barcode-match branch inside
    match_customer() now also checks src_barcode_is_learned so
    freshly-computed rows are flagged, and (b) finalize_barcode_fields()
    re-derives 'Borrowed Barcode' for EVERY row (fresh or cached) from the
    final src_barcode_is_learned state, exactly the same way FIX 3 already
    does for 'Our Barcode' / 'Barcode Source' — so this is immune to
    processing order and cache hits too.
  ★ VERSION TAG — a SCRIPT_VERSION constant is printed at startup so you
    can always confirm which script actually executed, from the console
    output alone.

Everything else — candidate bucketing, the fuzzy ensemble scorer, the
semantic layer + its .npy/.json cache, the barcode↔name bridge, the brand
pre-gate, the filter/scoring function, mgrp_descr-first brand resolution,
and the output workbook layout — is UNCHANGED from V6.
"""

SCRIPT_VERSION = "V6.1-fixed (standalone catalogue + barcode-poisoning fixes + borrowed-barcode fix)"
...
# ── CUSTOMER FILE (this holds every customer, including LULU's rows, which
#    are now matched like any other customer) ──────────────────────────────
...
def load_input_dataframe() -> pd.DataFrame:
    """Loads the combined CUSTOMER file (LULU included, as just another
    customer), using a Parquet cache under DATA_DIR keyed off size+mtime."""
...
def match_customer(
    ...
    for ci in range(len(cust)):
        ...
        if ci in barcode_matched_cis:
            ...
            # ★ FIX 6 — even when this exact barcode was learned in a PAST
            # pass (this run or a previous cached run), _maybe_backfill()
            # above short-circuits to (False, None, False) for plain
            # 'Barcode' / 'Barcode (Bridged)' methods, since it only teaches
            # NEW barcodes. That left 'Borrowed Barcode' blank even though
            # the catalogue row's barcode is itself Learned (Backfilled) —
            # i.e. the "match" is really against a barcode that was never
            # in the original source-of-truth catalogue. Flag it here so
            # it's visible.
...
# ===========================================================================
# ★ FIX 3 / ★ FIX 6 — finalize barcode fields AFTER all customers have run,
# so every matched row shows the FINAL, fully-taught barcode state
# regardless of customer processing order or cache hits. This now also
# re-derives 'Borrowed Barcode' for Barcode/Bridged rows whose catalogue
# barcode turned out to be Learned (Backfilled) — the same staleness
# problem FIX 3 already solved for 'Our Barcode' / 'Barcode Source', just
...
def finalize_barcode_fields(rows, src_barcode_display, src_barcode_is_learned):
    for row in rows:
        ...
        # ★ FIX 6 — for rows matched directly via barcode lookup (Method =
        # 'Barcode' or 'Barcode (Bridged)'), if the catalogue row's barcode
        # is itself Learned (Backfilled) — taught by some other customer,
        # maybe in a later pass or a different run — surface that in
        # 'Borrowed Barcode' too, even if it was left blank when the row
        # was first computed/cached.
...
def write_all_results(
    ...
    with pd.ExcelWriter(OUTPUT_FILE, engine='openpyxl') as writer:
        ...
        # ★ FIX 4 — Learned Barcodes Audit: every backfilled barcode, who
        # taught it, and at what confidence, so a bad learn is reviewable
        # instead of silent.
...
def main():
    ...
    # ── Load the CUSTOMER file (LULU included — it's just another customer now)
    ...
    # Every distinct source_customer label is now a customer, LULU included.
    customer_labels = sorted(df['source_customer'].unique())
    ...
    # ★ FIX 3 / ★ FIX 6 — now that EVERY customer (fresh or cached) has had a
    # chance to teach barcodes, re-resolve 'Our Barcode' / 'Barcode Source' /
    # 'Borrowed Barcode' on every matched row from the final, fully-taught
    # state. This is what actually eliminates the stale-blank-barcode
    # symptom (and the stale-blank-Borrowed-Barcode symptom), independent
    # of processing order or cache hit/miss.
