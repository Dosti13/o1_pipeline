"""
MatchIQ v2 — Fixed Weight Logic + Brand Matching
=================================================

Brand matching summary:
  • Source brand column : mgrp_descr
  • Target brand column : brand
  • Brand fuzzy threshold: 75
  • Brand match is FUZZY ONLY (not TF-IDF or semantic)
  • Brand Match column values:
      Exact    → brand strings match exactly (after normalization)
      Fuzzy    → matched via fuzzy (e.g. tiff → tiffany)
      Mismatch → no brand match found
      N/A      → brand missing on one or both sides
  • Brand mismatch does NOT block a description match — it only flags it
  • Alias dictionary (discovered fuzzy brand pairs) written to Excel sheet

Run:  python matchiq_v2_brand.py
"""
...
CONFIG = {
    "source": {
        ...
        "brand_column": "mgrp_descr",          # ← source brand column
    },
    ...
    "targets": [
        {
            ...
            "column":       "Item Name",
            "brand_column": "Brand",            # ← target brand column
        },
    ],

    "output": r"C:\Users\HP\Downloads\matchiq_v2_brand_results.xlsx",
    ...
    "thresholds": {
        ...
        "tfidf":    80,
        "semantic": 80,
        "brand":    75,                         # ← brand fuzzy threshold
    },
...
UNIT_MAP = {
    ...
    r"\bgram[s]?\b": "g",        r"\bgrm[s]?\b": "g",       r"\bgr?\b": "g",
    r"\bgm[s]?\b": "g",          r"\bG\b": "g",
...
# Matches pack+weight patterns like 4x12x50g, 24x81g, 25gx16
PACK_WEIGHT_RE = re.compile(
    r"(\d+\s*x\s*)+(\d+(?:\.\d+)?)\s*(g|gm|gr|grm|grams|kg|ml|l|ltr|oz)\b"
    r"|(\d+(?:\.\d+)?)\s*(g|gm|gr|grm|grams|kg|ml|l|ltr|oz)\s*x\s*\d+",
    re.IGNORECASE,
)
...
def parse_description(text: str) -> tuple:
    ...
    # First try to extract unit weight embedded in pack string e.g. 4x12x50g or 25gx16
    weight = ""
    pw_match = PACK_WEIGHT_RE.search(s)
    if pw_match:
        # group(2)+group(3) for NxNxWEIGHT pattern
        # group(4)+group(5) for WEIGHTxN pattern
        if pw_match.group(2) and pw_match.group(3):
            weight = pw_match.group(2) + pw_match.group(3)
        elif pw_match.group(4) and pw_match.group(5):
            weight = pw_match.group(4) + pw_match.group(5)
        s = s[:pw_match.start()] + " " + s[pw_match.end():]
    else:
        # Fallback: grab the last standalone weight token
    ...
    # Grab pack pattern e.g. 4x12, 24x, 2x3
    pack = ""
    pm = PACK_RE.search(s)
    if pm:
...
def normalize_weight(w: str) -> Optional[float]:
    ...
    if unit in ("g", "gm", "gms", "gr", "grm", "grms", "gram", "grams"): return val
...
def normalize_brand(b) -> str:
    """Lowercase + strip for brand comparison."""
...
def match_brand(
    ...
    """
    Returns: 'Exact' | 'Fuzzy' | 'Mismatch' | 'N/A'
    Also populates alias_dict with fuzzy brand pairs discovered.
    """
    sb = normalize_brand(src_brand)
    cb = normalize_brand(cust_brand)
    ...
    if score >= threshold:
        # Record the alias pair (source_brand → customer_brand)
        key = sb
        if key not in alias_dict:
            alias_dict[key] = {"Customer Brand": cb, "Fuzzy Score": score}
        return "Fuzzy"
...
def weight_priority(src_w: Optional[float], cust_w: Optional[float]) -> int:
    if src_w is not None and cust_w is not None:
        return 0 if src_w == cust_w else 3
...
class SemanticMatcher:
    def __init__(self, source_names: list, model_name: str, batch_size: int):
        ...
        self.embeddings = self.model.encode(
            source_names, batch_size=batch_size,
...
def read_file(path, col, brand_col=None):
    path = Path(path)
    ...
    # Brand column — fill with empty string if missing
    if brand_col:
        if brand_col in df.columns:
            df["__brand_col__"] = df[brand_col].fillna("").astype(str).str.strip()
        else:
            log.warning("  Brand column '%s' not found — brand will be N/A", brand_col)
            df["__brand_col__"] = ""
    else:
        df["__brand_col__"] = ""
...
def write_results(results: pd.DataFrame, output_path, alias_dict: dict):
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        # ── Brand summary
        b_counts = results["Brand Match"].value_counts().reset_index()
        b_counts.columns = ["Brand Match", "Count"]
        b_counts["Pct"]  = (b_counts["Count"] / total * 100).round(1).astype(str) + "%"
        b_counts.to_excel(writer, sheet_name="Brand Summary", index=False)

        # ── Brand alias dictionary (fuzzy pairs discovered)
        if alias_dict:
            alias_rows = [
                {
                    "Source Brand (mgrp_descr)": src,
                    "Customer Brand (brand)":    v["Customer Brand"],
            ...
            log.info("  Brand alias pairs discovered: %d", len(alias_rows))
        else:
            pd.DataFrame(
                columns=["Source Brand (mgrp_descr)", "Customer Brand (brand)", "Fuzzy Score"]
            ).to_excel(writer, sheet_name="Brand Alias Dictionary", index=False)
...
def run_pipeline(cfg: dict):
    ...
    # alias dict: source_brand → {Customer Brand, Fuzzy Score}
    alias_dict = {}

    # ── Source ──────────────────────────────────────────────────────────────
    src_brand_col = cfg["source"].get("brand_column", "")
    src_df, _     = read_file(
        cfg["source"]["file"],
        cfg["source"]["column"],
        brand_col=src_brand_col,
    )
    source_raw = src_df["__desc_col__"].tolist()

    log.info("Parsing %d source descriptions …", len(source_raw))
    source_parsed = []
    for i, raw in enumerate(source_raw):
    ...
    for tgt_cfg in cfg["targets"]:
        tgt_brand_col = tgt_cfg.get("brand_column", "")
        tgt_df, _     = read_file(
            tgt_cfg["file"],
            tgt_cfg["column"],
            brand_col=tgt_brand_col,
        )

        if sample_n is not None:
            tgt_df = tgt_df.head(sample_n)
            log.info("  → SAMPLE MODE: %d rows", sample_n)
        ...
        target_parsed = []
        for i, raw in enumerate(target_raw):
            name, pack, weight = parse_description(raw)
            target_parsed.append({
                ...
                "weight_raw": weight,
                "weight_n":   normalize_weight(weight),
                "brand":      tgt_df["__brand_col__"].iloc[i],
            })
        ...
        # Pre-encode semantic embeddings
        target_emb = None
        ...
        for i, tp in enumerate(tqdm(target_parsed,
            ...
            # ── Brand match (fuzzy only, always runs if match found) ──────
            brand_label = "N/A"
            src_brand   = ""
            if best["idx"] is not None:
                src_brand   = source_parsed[best["idx"]]["brand"]
                brand_label = match_brand(src_brand, cust_brand, brand_th, alias_dict)
    ...
    log.info("  Brand alias pairs : %d", len(alias_dict))
    for src, v in sorted(alias_dict.items()):
        log.info("    %-20s → %-20s (score=%d)", src, v["Customer Brand"], v["Fuzzy Score"])
    log.info("  Time        : %.1fs", time.time() - t0)
    log.info("=" * 60)

    write_results(final, cfg["output"], alias_dict)
    return final
...
def main():
    ...
    log.info("MatchIQ v2 (brand matching) starting …")
    ...
    log.info("  Brand thresh: %d", cfg["thresholds"]["brand"])
    log.info("  Sample      : %s",
